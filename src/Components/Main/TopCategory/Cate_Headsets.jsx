...
import { headsetProducts } from '../../../Data'
import 'animate.css';
  ...
  return (
    <div className='w-full h-auto px-6 md:px-14'>
         <div className='w-full flex items-center justify-between pt-10 border-b py-3'>
            <h1 className='text-3xl  text-center font-bold  uppercase '>Product Of Category Headsets</h1>
        ...
        <div className='w-full flex flex-wrap gap-8 md:gap-6 lg:gap-9 py-10 h-auto'>
                {/* card */}
                {headsetProducts.map((p,i)=>(
                    <div key={p.id} 
                        onClick={()=>Navigate(`/headsets/${p.id}`)}
